...
After editing Python files (like `GdDS/routes.py`):

```bash
python -m py_compile GdDS/__init__.py  # Check single file for syntax errors
```
...
- Backend processing in `GdDS/routes.py`
...
- **✅ Do**: `GdDS/` with underscores, all lowercase
...
export namespace CommandIDs {
  export const openPanel = 'GdDS:open-panel';
  export const refreshData = 'GdDS:refresh-data';
}
...
- Backend routes: `GdDS/routes.py` (class `DataRouteHandler`)
- Frontend logic: `src/` directory
- Python package: `GdDS/` directory
...
1. **Read the backend first** — Check `GdDS/routes.py` to understand the existing API contract
...
Create RESTful endpoints in `GdDS/routes.py`:
...
def setup_route_handlers(web_app):
    ...
    data_route = url_path_join(base_url, "GdDS", "data")
...
- Define matching endpoint path strings (e.g., `"hello"`, `"get-data"`) in both `GdDS/routes.py` and `src/api.ts` to ensure routes sync between backend and frontend
...
2. **Write the code** (TypeScript in `src/`, styles in `style/`, Python in `GdDS/`)
...
- **Output**: Creates files in `lib/` and `GdDS/labextension/`
...
**After editing Python** (files in `GdDS/`):
...
- Ask user to search for the extension ID (`GdDS`) to see if it loaded
...
ls -la GdDS/labextension/  # Should contain bundled extension
```
...
- Backend routes: `GdDS/routes.py`
- Server extension config: `jupyter-config/server-config/GdDS.json`
...
**❌ Don't**: Write frontend interfaces without first checking the backend API contract in `GdDS/routes.py`
...
**❌ Don't**: Use absolute imports like `from GdDS.routes import ...`
...
- **Plugin ID** (in `src/index.ts`): `'GdDS:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'GdDS:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-GdDS-ClassName`
- **API routes** (in `GdDS/routes.py`): `url_path_join(base_url, "GdDS", "endpoint")`
