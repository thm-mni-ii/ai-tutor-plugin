...
**3. A mild follow-up question leaked the full fix.** Asking "I don't understand, what's wrong with my code?" got back the complete, working corrected line — a direct violation of rule 3. Cause: follow-ups only get a short grounding message, not the full rule set, so the anti-solution instruction simply wasn't present by the second turn.
  ...
  - Third attempt — ban writing the corrected expression *in any form at all* (fenced, inline, or spelled out), with an explicit self-check ("if the student could copy-paste any part of your answer to fix it, you've gone too far") — worked on the first retest, but **leaked again on a later live run** with the same question, just reworded ("Die korrekte Form wäre: ..."). Same instruction, inconsistent compliance — a genuine reliability limit, not a wording gap.
  - Fourth attempt — kept the same instruction, but repeated a short version of it immediately after the student's question itself (not just once, earlier in the conversation) — recency in the context appears to matter more than having stated the rule at all. Re-ran the identical leaking scenario **four times**; held clean on all four. Meaningfully more evidence of reliability, though with a non-deterministic model this is confidence, not a hard guarantee.
...
- **Prompting alone doesn't give a 100% guarantee.** The same explicit instruction held on some calls and leaked on others, on identical input — a real, model-level reliability limit, not something more careful wording fully closes. For a production deployment (beyond this project's scope), the honest next step would be a code-side check on the response itself, not just relying on the prompt.
